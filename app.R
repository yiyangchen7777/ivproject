...
# =========================================================
# app.R final version: Welcome + main page + suspendWhenHidden fix + Navbar(Map/Route/Detail)
# =========================================================
...
# Load the Tableau script when present (no error if missing)
...
# ===================== Original opening-hours helper (kept unchanged) =====================
...
ui <- fluidPage(
  ...
  tags$head(
    ...
    tags$style(HTML("
      ...
      #welcome {
        position: relative; z-index: 3; height:100vh; min-height:100vh;
      ...
      #mainApp {
        ...
        display: block; /* Keep the element available but hidden */
      }
      .glass {
        ...
        width:90%; max-width:1200px; height:90vh; max-height:900px; display:flex; flex-direction:column; align-items:center; justify-content:flex-start;
        overflow: hidden;
      }
      .glass iframe {
        display: block;
      ...
      .persona-section {margin-top:24px; display:grid; grid-template-columns:repeat(auto-fit, minmax(260px, 1fr)); gap:16px;}
      .persona-card {background:#ffffff; border-radius:14px; box-shadow:0 6px 18px rgba(15,23,42,0.08); padding:18px 20px; display:flex; flex-direction:column; gap:10px;}
      .persona-card h5 {margin:0; font-size:16px; font-weight:600; color:#1f2933;}
      .persona-card .persona-role {font-size:12px; font-weight:600; letter-spacing:0.4px; text-transform:uppercase; color:#6366f1;}
      .persona-card p {margin:0; font-size:13px; line-height:1.55; color:#475467;}
    ")),
    tags$style(HTML("
        .hero-card,
        .meta-card,
    ...
    tags$script(HTML("
      function goToMain(){
        ...
        $('#mainApp').css({opacity:1,pointerEvents:'auto', visibility:'visible'});
        setTimeout(function(){
      ...
      $(document).ready(function(){
        // Ensure the welcome screen shows first and the main app stays hidden
        // Preserve the URL hash so Shiny's routing continues to work
        $('#welcome').css({opacity: 1, pointerEvents: 'auto', display: 'flex', visibility: 'visible'});
        $('#mainApp').css({opacity: 0, pointerEvents: 'none', visibility: 'hidden'});
    ...
    <style>
      ...
      /* Tighten spacing between dropdowns and section titles */
      #filter-panel .shiny-input-container {
        margin-top: 4px !important;
        margin-bottom: 1px !important;
      }
      
      /* Global: remove fluidPage default padding so edges stay tight */
      .container-fluid {
        padding: 0 !important;
        margin: 0 !important;
        width: 100%;
      ...
      /* Sidebar panel: fixed narrow width for a tighter layout */
      .col-sm-3 {
      ...
      /* Select Ratings card styling (matches Select Categories) */
      #rating-section {
        background-color: #f8f9fa;
        border-radius: 10px;
        padding: 10px 12px;
      ...
      /* Reduce spacing between the dropdown and heading */
      #rating-section .shiny-input-container {
        margin-top: 4px !important;
        margin-bottom: 4px !important;
      }
      
      /* Trim padding within the dropdown control */
      ...
      /* Reduce spacing between checkboxes and the heading */
      #open-filter .shiny-input-container {
        margin-top: 4px !important;
        margin-bottom: 4px !important;
      }
      
      /* Adjust checkbox font size and line height */
      #open-filter label {
        font-size: 14px;
        font-weight: 600;
        margin-bottom: 2px;
      ...
      /* Select Regional Range card styling */
      #regional-range {
        background-color: #f8f9fa;
        border-radius: 10px;
        padding: 10px 12px;
      ...
      /* Primary action button: Locate Me (blue) */
      #locate_btn {
      ...
      /* Secondary action button: Clear (grey-red) */
      #clear_btn {
      ...
      /* Natural spacing between the two buttons */
      ...
      /* Add more space between the dropdown and buttons */
      #regional-range .shiny-input-container {
        margin-bottom: 12px !important;
      }
  
      /* mainPanel: fill the remaining space */
      .col-sm-9 {
        flex: 1 1 auto;
      ...
      /* Map container: fill the right-hand column */
      #map-container {
      ...
      /* Make leafletOutput responsive to its container */
      #map {
        width: 100% !important;
        height: 100% !important;
      }
  
      /* Slightly reduce spacing between titles and content */
      h2, .title {
        margin-bottom: 20px;
      }

      /* Glow animation (cyan) only for venues, not the user marker */
      ...
      /* Broaden button letter spacing for balance */
      #locate_btn span, #clear_btn span {
        letter-spacing: 0.3px;
      }
  
      /* Keep buttons centered when sharing one row */
      ...
      /* Align content horizontally within the row */
      .shiny-row {
        display: flex;
        gap: 8px;
      }
      
      /* ===== Map loading animation styles ===== */
      #map-container {
        position: relative;
      }
    ...
    <script>
      ...
      // When All is checked, clear other options and collapse subcategories
      ...
      // Parent categories are mutually exclusive and toggle subgroups
      ...
      // Store markers glow; skip the user-location marker
      let lastActive = null;
      ...
      // 📍 Locate Me (high accuracy, no caching)
  ...
  div(
    ...
    div(
      ...
      div(
        style="flex:1;width:100%;min-height:0;display:flex;justify-content:center;align-items:center;background:rgba(255,255,255,0.08);border-radius:20px;padding:20px;box-sizing:border-box;overflow:auto;position:relative;",
        HTML("
          <iframe 
            ...
            frameborder='0'
            style='border:none;background:transparent;border-radius:16px;width:100%;height:100%;max-width:100%;max-height:100%;display:block;margin:0 auto;'
            scrolling='auto'
  ...
  div(
    ...
    navbarPage(
      ...
      tabPanel(
        ...
        # Dynamic main content: either full-screen skyline or full detail layout
        uiOutput("mainContent")
      ),
      
      
      # ====== Original search-page UI (as-is) ======
...
server <- function(input, output, session){
  ...
  # ---------- Skyline image from static assets (www/melbourne-skyline.jpg) ----------
  skyline_file <- "sky-line.jpg"
  if (!file.exists(file.path("www", skyline_file))) {
    skyline_file <- NULL  # Use an online fallback when not in www
  }
  
  # ---------- State ----------
  last_pick    <- reactiveVal(NULL)  # Track the most recent valid selection
  has_selected <- reactiveVal(FALSE) # Whether a venue has ever been selected
  show_skyline <- reactiveVal(TRUE)  # Whether to show the full-screen skyline (initially TRUE)
  user_loc     <- reactiveVal(NULL)  # Geolocation state for the Map tab
  
  route_module_server("route", map_user_location = reactive(user_loc()))
  
  # Observe changes to the search box
  observeEvent(input$pick, ignoreInit = TRUE, {
    if (!is.null(input$pick) && nzchar(input$pick)) {
      ...
      has_selected(TRUE)
      show_skyline(FALSE)   # Show detail once a selection has been made
    } else {
      # On clear keep the detail tab visible and hide the skyline
      # (Uncomment below to restore the last selection)
      # updateSelectizeInput(session, "pick", selected = last_pick(), server = TRUE)
    }
  })
  
  # ✅ Change: always show the skyline when returning from Map/Route to Detail
  ...
  observeEvent(input$navtabs, ignoreInit = TRUE, {
    if (!identical(input$navtabs, "Route")) return()
    session$sendCustomMessage("route_refresh_map", list())
  })

  # Triggered by the Route page "See Details" action
  ...
  # Selected record (fall back to last_pick when empty)
  sel <- reactive({
  ...
  # ================= Main content: full-screen skyline or detail layout =================
  output$mainContent <- renderUI({
    ...
    tagList(
      ...
      div(
        ...
        div(
          class = "persona-card",
          div(class = "persona-role", "City Explorer"),
          h5("Sophia Chen — International Student"),
          p("Sophia uses the detail view to shortlist brunch spots before weekend catch-ups. The photo carousel helps her check the vibe, while the price and rating cards confirm it suits her student budget. She keeps an eye on the opening-hours panel and nearby peers map so she can suggest back-up venues if the first choice is packed.")
        ),
        div(
          class = "persona-card",
          div(class = "persona-role", "Experience Curator"),
          h5("Liam Patel — Boutique Hotel Concierge"),
          p("Liam leans on the detail interface when crafting itineraries for hotel guests. He reviews the narrative description to match venues with guest preferences, verifies accessibility via the peer map, and copies the contact details to confirm table availability before sending a personalised recommendation.")
        )
      )
    )
  })
  ...
  output$shopImage <- renderUI({
    # ⚠️ Removed the "show skyline when pick is empty" behavior; this only drives the carousel
    s <- sel()
  ...
  observe({
    ...
    popup_html <- function(row) {
      ...
      # Add an "Add to Route" button
      venue_name_encoded <- URLencode(row$name, reserved = TRUE)
      add_to_route_btn <- sprintf(
        '<button onclick="Shiny.setInputValue(\'route-add_from_map\', \'%s\', {priority:\'event\'})" style="margin-top:10px; padding:8px 15px; background-color:#3478f6; color:white; border:none; border-radius:6px; cursor:pointer; font-weight:500; width:100%%; letter-spacing:0.5px; transition:all 0.2s ease;">➕ Add to Route</button>',
        venue_name_encoded
      )
      sprintf(
        "<b>%s</b><br/>%s<br/>%s<br/>%s<br/>%s<br/>%s<br/>%s<br/>%s",
        htmlEscape(row$name),
